...
class Feeder(Dataset):
    ...
    def load_data(self):
        ...
        for fname in tqdm(file_list, desc=f"Loading {self.mode} samples"):
            ...
            try:
                ...
                with open(length_path, 'rb') as f:
                    length = pickle.load(f, encoding='latin1')
        ...
        self.data = np.concatenate(data_list, axis=0)      # shape [T_total, 150]
        ...
        for start in self.windows:
            ...
            self.window_to_video_idx.append(vid_idx)
        self.window_to_video_idx = np.array(self.window_to_video_idx, dtype=np.int64)
...
def main():
    ...
    DATA_DIR = "/home/user/Annisa/MS-G3D/data/pkummd/Data/PKU_Skeleton_Renew"
    LABEL_DIR = "/home/user/Annisa/MS-G3D/data/pkummd/Label"
    SPLIT_PATH = "/home/user/Annisa/MS-G3D/data/pkummd/Split/cross-subject.txt"
    LENGTH_DIR= "/home/user/Annisa/MS-G3D/data/pkummd/Length"
    ...
    for i, (data, label, distance, total_len, idx) in enumerate(train_loader):
        print(f"Batch {i} | Data: {data.shape} | Label: {label.shape} | Distance: {distance.shape} | Len: {total_len.shape}")
        print(data.shape, label, distance, total_len, idx)
        if i == 1:
            break

    print("\n--- Testing Val Loader ---")
    for i, (data, label, distance, total_len, idx) in enumerate(val_loader):
        # print(f"Sample {i} | Data: {data.shape} | Label: {label.shape} | Distance: {distance.shape} | Len: {total_len.shape}")
        print(data.shape, label, distance, total_len, idx)
        if i == 5:
            break
    ...
    # sample_data, sample_label, distance, total_len, idx = train_dataset[1002]
