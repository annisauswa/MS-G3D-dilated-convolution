    ...
    def train(self, epoch, save_model=False):
        ...
        for batch_idx, (data, label, distance, total_len, index) in enumerate(process):
            ...
            for i in range(splits):
                ...
                # forward
                output, s_pred = self.model(batch_data, distance_norm)
                if ((epoch + 1) % 1 == 0):
    ...
    def eval(self, epoch, save_score=False, loader_name=['test'], wrong_file=None, result_file=None):
        ...
        with torch.no_grad():
            ...
            if self.arg.phase == 'train' and not self.arg.debug:
                ...
                self.val_writer.add_scalar('regression_rmse', epoch_rmse, self.global_step)
                self.val_writer.add_scalar('regression_mae', epoch_mae, self.global_step)
