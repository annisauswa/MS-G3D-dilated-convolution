...
from model.ssnet2 import SSNet
...
class Model(nn.Module):
    def __init__(self,
        ...
        self.ssnet = SSNet(384, 96, num_layers=7,pad_mode='zero')

        self.fc = nn.Linear(c3, num_class)

    def forward(self, x, st_prev):
        N, C, T, V, M = x.size()
        ...
        out_channels = out.size(2)
        # print('1:', out.shape)
        out = out.view(N, M, out_channels, out.size(1), out.size(3))
        # print('2:', out.shape)
        out = out.mean(1)
        # print('3:', out.shape)
        out = out.mean(3)
        ...
        # print('5:', out.shape)
        out = self.ssnet(out, st_prev)
        # print('6:', out[0].shape, out[1].shape)
